...
#####P-value cut-off 5E-6###
dat3=dat[dat$pval.exposure<5e-6,]
mr_results3 <- mr(dat3,method_list=c("mr_ivw_mre",
                                     "mr_weighted_median",
                                     "mr_egger_regression"))
het3=mr_heterogeneity(dat3)
ple3=mr_pleiotropy_test(dat3)
OR_mr_results3=generate_odds_ratios(mr_results3)

###MR-PRESSO ANALYSIS###
mr_presso3=run_mr_presso(dat3, NbDistribution = 2500, SignifThreshold = 0.05)

######META-ANALYSIS###
data_meta=mr_results3[mr_results3$method=="Inverse variance weighted (multiplicative random effects)",]
meta_CAD=data_meta[grep("CAD",data_meta$outcome),]
m_CAD <-metagen(meta_CAD$b,meta_CAD$se,studlab=meta_CAD$outcome, sm="BETA",backtransf=TRUE)
m_CAD
...
#####P-value cut-off 5E-5###
dat4=dat[dat$pval.exposure<5e-5,]
mr_results4 <- mr(dat4,method_list=c("mr_ivw_mre",
                                     "mr_weighted_median",
                                     "mr_egger_regression"))
het4=mr_heterogeneity(dat4)
ple4=mr_pleiotropy_test(dat4)
OR_mr_results4=generate_odds_ratios(mr_results4)

###MR-PRESSO ANALYSIS###
mr_presso4=run_mr_presso(dat4, NbDistribution = 2500, SignifThreshold = 0.05)

######META-ANALYSIS###
data_meta=mr_results4[mr_results4$method=="Inverse variance weighted (multiplicative random effects)",]
meta_CAD=data_meta[grep("CAD",data_meta$outcome),]
m_CAD <-metagen(meta_CAD$b,meta_CAD$se,studlab=meta_CAD$outcome, sm="BETA",backtransf=TRUE)
m_CAD
